...
What tree_depth and min criterion values created highest accuracy?  # the higher the better
...
```
Let's select the best model below. # the higher the better
...
final_fit %>%
  collect_predictions()
```
# 211 rows came from the test data
...
final_fit %>%
  collect_metrics()
```
# Accuracy = The higher the better it is. How many rows model got right
...
# augument help to get predictions
# high because we used train set
...
```
# 1 because we get 100% all the values in the diagonal. They divided by themselves.
...
# A perfect model goes all the way until 1.00 for 100%. All three lines overlapping.
